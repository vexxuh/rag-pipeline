(function () {
  ...
  var styles =
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }\
    ...
    .rag-msg { max-width: 85%; padding: 10px 14px; border-radius: 12px; line-height: 1.4; word-wrap: break-word; overflow-wrap: anywhere; }\
    .rag-msg-user { align-self: flex-end; color: white; border-bottom-right-radius: 4px; white-space: pre-wrap; }\
    .rag-msg-assistant { align-self: flex-start; background: #f1f3f5; color: #333; border-bottom-left-radius: 4px; white-space: normal; }\
    .rag-msg-greeting { align-self: flex-start; background: #f1f3f5; color: #333; border-bottom-left-radius: 4px; white-space: normal; }\
  ...
  // Escape HTML entities for safe innerHTML during streaming
  ...
  var mdStyles =
    "\
    .rag-msg-assistant code { background: rgba(0,0,0,0.06); padding: 1px 4px; border-radius: 3px; font-size: 0.9em; }\
    .rag-msg-assistant pre { background: rgba(0,0,0,0.06); padding: 8px; border-radius: 6px; overflow-x: auto; margin: 4px 0; }\
    .rag-msg-assistant pre code { background: none; padding: 0; }\
    .rag-msg-assistant strong { font-weight: 600; }\
    .rag-msg-assistant ul, .rag-msg-assistant ol { padding-left: 18px; margin: 4px 0; }\
    .rag-msg-assistant li { margin: 2px 0; }\
    .rag-msg-greeting code { background: rgba(0,0,0,0.06); padding: 1px 4px; border-radius: 3px; font-size: 0.9em; }\
  ";
  ...
  function createWidget() {
    ...
    styleEl.textContent = styles + mdStyles;
    shadow.appendChild(styleEl);

    // Inject custom CSS from config (after base styles so it can override)
  ...
  function addMessage(role, content, extraClass) {
    ...
    msg.className =
      "rag-msg " +
      (extraClass || (role === "user" ? "rag-msg-user" : "rag-msg-assistant"));
    if (role === "user") {
      msg.textContent = content;
      msg.style.background = config.primary_color;
    } else {
      msg.innerHTML = simpleMarkdown(content);
    }
  ...
  async function sendMessage() {
    ...
    try {
      ...
      var res = await fetch(
        SERVER + "/api/widget/conversations/" + convId + "/messages",
        {
          method: "POST",
          headers: apiHeaders(),
      ...
      while (true) {
        ...
        for (var i = 0; i < lines.length; i++) {
          ...
          // Strip single leading space after "data:" per SSE spec, preserve all other whitespace
          ...
          // Use innerHTML with escaped text, converting newlines to <br> for proper display
          assistantMsg.innerHTML = escapeHtml(assistantContent).replace(
            /\n/g,
            "<br>",
          );
          msgList.scrollTop = msgList.scrollHeight;
      ...
      if (assistantContent) {
        ...
        if (assistantMsg) {
          assistantMsg.innerHTML = simpleMarkdown(assistantContent);
        }
    ...
    try {
      ...
      if (res.ok) {
        ...
        config.greeting_message =
          data.greeting_message || config.greeting_message;
  ...
  async function loadHistory() {
    ...
    try {
      var res = await apiFetch(
        "/api/widget/conversations/" + convId + "/messages",
      );
      if (res.ok) {
        var data = await res.json();
        if (data.length > 0) {
