...
pub async fn list_providers(
    ...
    let providers = state.admin_config_repo.get_enabled_providers()?;
    Ok(Json(providers))
}
...
pub async fn list_api_keys(
    ...
    let keys = state.settings_repo.list_api_keys(&claims.sub)?;
    Ok(Json(keys))
}
...
pub async fn set_api_key(
    ...
    let entry = state
        .settings_repo
        .set_api_key(&claims.sub, &provider, &payload.api_key)?;
...
pub async fn delete_api_key(
    ...
    state.settings_repo.delete_api_key(&claims.sub, &provider)?;
    Ok(())
}
...
pub async fn update_preferences(
    ...
    state
        .settings_repo
        .set_preferences(&claims.sub, &payload)?;
